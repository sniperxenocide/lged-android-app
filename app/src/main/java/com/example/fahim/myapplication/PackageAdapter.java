...
public class PackageAdapter extends ArrayAdapter<JSONObject> implements View.OnClickListener

{
    ...
    public PackageAdapter(ArrayList<JSONObject> data, Context context, MainActivity ma)
    {
    ...
    public void detailButtonFunction(JSONObject o, int p)
    {
        ...
        try
        {
            ...
            f1.setText("APP ID");
            x = o.getString("app_id_value");
            if(x!="null")  f2.setText(x);

            g1.setText("Package No.");
            x = o.getString("package_no");
            if(x!="null") g2.setText(x);

            h1.setText("Package Description");
            x = o.getString("package_description");
            if(x!="null") h2.setText(x);

            i1.setText("Package Estimation Cost");
            x = o.getString("app_cost");
            if(x!="null") i2.setText(x);

            j1.setText("Lot No.");
            x = o.getString("lot_no");
            ...
            x = o.getString("Package_status");
            if(x!="null") u2.setText(x);

            v1.setText("Package Type");
            x = o.getString("Package_type");
    ...
    public View getView(final int position, View convertView, ViewGroup parent)
    {
        ...
        final JSONObject Package = getItem(position);
        final PackageAdapter.ViewHolder viewHolder;

        final View result;

        if (convertView == null) {

            viewHolder = new PackageAdapter.ViewHolder();
            ...
            viewHolder = (PackageAdapter.ViewHolder) convertView.getTag();
            result = convertView;
        }
        ...
        try {
            String x = "";
            if (Package.getString("office_name") != "null") x = Package.getString("office_name");
            viewHolder.t_name.setText(x);

            x="";
            if (Package.getString("package_description") != "null") x = Package.getString("package_description");
        ...
        viewHolder.detailButton.setOnClickListener(new View.OnClickListener()
        {
            ...
            {
                detailButtonFunction(Package,position);
            }
        });
