...
public class ContractAdapter extends ArrayAdapter<JSONObject> implements View.OnClickListener

{
    ...
    public ContractAdapter(ArrayList<JSONObject> data, Context context, MainActivity ma)
    {
    ...
    public void detailButtonFunction(JSONObject o, int p)
    {
        ...
        try
        {
            ...
            f1.setText("Tender ID");
            x = o.getString("tender_id_value");
            if(x!="null")  f2.setText(x);

            g1.setText("Physical Progress");
            x = o.getString("physical_progress");
            if(x!="null") g2.setText(x);

            h1.setText("Amount Paid");
            x = o.getString("amount_paid");
            if(x!="null") h2.setText(x);

            i1.setText("Actual Start Date");
            x = o.getString("start_date");
            if(x!="null") i2.setText(x);

            j1.setText("Expected Completion Date");
            x = o.getString("completion_date");
            if(x!="null") j2.setText(x);

            k1.setText("Revise Contract Amount");
            x = o.getString("revise_contract_amount");
            if(x!="null") k2.setText(x);

            l1.setText("Actual Completion Date");
            x = o.getString("actual_completion_date");
            ...
            x = o.getString("Contract_status");
            if(x!="null") u2.setText(x);

            v1.setText("Contract Type");
            x = o.getString("Contract_type");
    ...
    public View getView(final int position, View convertView, ViewGroup parent)
    {
        ...
        final JSONObject Contract = getItem(position);
        final ContractAdapter.ViewHolder viewHolder;

        final View result;

        if (convertView == null) {

            viewHolder = new ContractAdapter.ViewHolder();
            ...
            viewHolder = (ContractAdapter.ViewHolder) convertView.getTag();
            result = convertView;
        }
        ...
        try {
            String x = "";
            if (Contract.getString("office_name") != "null") x = Contract.getString("office_name");
            viewHolder.t_name.setText(x);

            x="";
            if (Contract.getString("status") != "null") x = Contract.getString("status");
        ...
        viewHolder.detailButton.setOnClickListener(new View.OnClickListener()
        {
            ...
            {
                detailButtonFunction(Contract,position);
            }
        });
