...
public class BudgetAdapter extends ArrayAdapter<JSONObject> implements View.OnClickListener

{
    ...
    private static class ViewHolder
    {
        TextView t_name;
        TextView status;
        Button budgetDetailButton;
    }

    public BudgetAdapter(ArrayList<JSONObject> data, Context context, MainActivity ma)
    {
    ...
    public void detailButtonFunction(JSONObject o, int p)
    {
        ...
        try
        {
            ...
            f1.setText("Fund Disburse From");
            x = o.getString("fund_disburse_from");
            if(x!="null")  f2.setText(x);

            g1.setText("Memo No.");
            x = o.getString("memo_no");
            if(x!="null") g2.setText(x);

            h1.setText("Financial Year");
            x = o.getString("financial_year");
            if(x!="null") h2.setText(x);

            i1.setText("Installment No.");
            x = o.getString("installment_no");
            if(x!="null") i2.setText(x);

            j1.setText("Issue Date");
            x = o.getString("issue_date");
            if(x!="null") j2.setText(x);

            k1.setText("Subject");
            x = o.getString("subject");
            if(x!="null") k2.setText(x);

            l1.setText("Total Provision");
            x = o.getString("total_provision");
            if(x!="null") l2.setText(x);

            m1.setText("Amount");
            x = o.getString("budget_amount");
            ...
            x = o.getString("budget_status");
            if(x!="null") u2.setText(x);

            v1.setText("budget Type");
            x = o.getString("budget_type");
    ...
    public View getView(final int position, View convertView, ViewGroup parent)
    {
        ...
        final JSONObject budget = getItem(position);
        final BudgetAdapter.ViewHolder viewHolder;
        ...
        if (convertView == null) {
            ...
            viewHolder.budgetDetailButton = (Button) convertView.findViewById(R.id.detailBtn);
        ...
        try {
            String x = "";
            if (budget.getString("office_name") != "null") x = budget.getString("office_name");
            viewHolder.t_name.setText(x);

            x="";
            if (budget.getString("financial_year") != "null") x = budget.getString("financial_year");
            if (budget.getString("budget_amount") != "null") x = x+ " | "+ budget.getString("budget_amount");
        ...
        viewHolder.budgetDetailButton.setOnClickListener(new View.OnClickListener()
        {
            public void onClick(View v)
            {
                detailButtonFunction(budget,position);
            }
        });
